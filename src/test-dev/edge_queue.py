...
class EdgeQueue:
	...
	def est_latency (cls, task):
		...
		total_latency = cls.__est_wait_time () + cls.__est_srv_time (task)
		
		return ResponseTime (total_latency, 0, 0, total_latency)


	def __est_util (cls, workloads):
	...
	def __est_srv_time (cls, task):

		avail = cls._total - cls._util
		
		return task.get_data_in () / (avail * math.log (1 + SNR, 2))
